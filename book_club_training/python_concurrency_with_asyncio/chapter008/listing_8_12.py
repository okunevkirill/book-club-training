...
class ServerState:
    ...
    async def _on_connect(self, writer: StreamWriter):
        writer.write(
            "Добро пожаловать! Число подключенных пользователей:"
            f" {len(self._writers)}!\n".encode()
        )
        await writer.drain()
    ...
    async def _echo(self, reader: StreamReader, writer: StreamWriter):
        try:
            ...
            logging.exception("Ошибка чтения данных от клиента.", exc_info=err)
            self._writers.remove(writer)
    ...
    async def _notify_all(self, message: str):
        for writer in self._writers:
            try:
                ...
                logging.exception("Ошибка записи данных клиенту.", exc_info=err)
                self._writers.remove(writer)
